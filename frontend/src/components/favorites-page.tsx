...
import { getUserFavorites, getPostImages } from "@/lib/api"
  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      ...
      try {
        ...
        if (res.status === "success" && Array.isArray(res.favourites)) {
          const posts = await Promise.all(
            res.favourites.map(async (fav: any) => {
              try {
                const postRes = await getPostById(fav.post_id)
                const post = postRes.post
          
                const imgRes = await getPostImages(fav.post_id)
                ...
                console.error(`Lỗi khi lấy bài viết ${fav.post_id}:`, error)
                return null
              }
            })
          )
