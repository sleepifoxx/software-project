...
interface SearchResultItemProps {
  ...
  activeNotification: { id: number; type: "success" | "error"; message: string } | null
}

function SearchResultItem({ item, viewMode,activeNotification,onToggleFavorite }: SearchResultItemProps) {
  if (viewMode === "list") {
    return (
      <Card className="overflow-hidden group">
        <div className="relative">
          <div className="aspect-video relative overflow-hidden">
            ...
            <div className="absolute top-2 right-2 flex gap-2">
              ...
              >
                <Heart
                  className={`h-4 w-4 ${
                    item.isFavorite ? "fill-primary text-primary" : "text-gray-700"
                  }`}
                />
              </Button>
          ...
          {activeNotification && activeNotification.id === item.id && (
            <div
              className={`mx-4 mt-2 text-xs text-center py-2 rounded-md ${
  ...
  return (
    <Card className="overflow-hidden group">
      <div className="relative">
        ...
        {activeNotification && activeNotification.id === item.id && (
  <div className={`mr-2 py-1 px-2 text-xs rounded-md flex items-center ${
    activeNotification.type === "success"
  ...
  onClick={() => onToggleFavorite(item.id)}
>
  <Heart className={`h-4 w-4 ${item.isFavorite ? "fill-primary text-primary" : "text-gray-700"}`} />
</Button>
          <Button size="icon" variant="secondary" className="h-8 w-8 rounded-full">
            <Share className="h-4 w-4" />
          </Button>
      ...
      <CardContent className="p-4">
        <div className="flex justify-between items-center mt-1">
          <p className="font-medium text-primary">{new Intl.NumberFormat("vi-VN").format(item.price)}đ/tháng</p>
          <p className="text-sm text-muted-foreground">{item.area}m²</p>
        </div>
        <p className="text-sm text-muted-foreground flex items-center mt-1">
          <MapPin className="h-3 w-3 mr-1" />
