...
def parse_price_string(price_str: str) -> int:
    """
    Convert string like '6,5 triệu/tháng' → 6500000
    """
    try:
        if "triệu" in price_str.lower():
            num = re.findall(r"[\d,\.]+", price_str)[0]
            num = num.replace(",", ".")
...
async def get_posts_by_limit(limit: int, offset: int = 0, db: AsyncSession = Depends(get_db)):
    if limit <= 0:
        return {"status": "fail", "message": "Limit must be greater than 0"}
    result = await db.execute(select(Posts).order_by(desc(Posts.id)).offset(offset).limit(limit))
    posts = result.scalars().all()
    return {"status": "success", "posts": posts}

@app.get("/search-posts", tags=["Posts"])
async def search_posts(
    location: str = Query(default="", alias="location"),
    price: str = Query(default="", alias="price"),
    ...
    min_price, max_price = price_ranges.get(price, (0, 999999999))

    filtered_posts = []
    for post in all_posts:
        # So khớp địa chỉ
        match_location = location.lower() in post.address.lower()

        # Chuyển đổi giá từ chuỗi → số
        price_number = parse_price_string(post.price)
        match_price = min_price <= price_number <= max_price

        if (not location or match_location) and (not price or match_price):
            filtered_posts.append(post)

    return {"status": "success", "posts": filtered_posts}
